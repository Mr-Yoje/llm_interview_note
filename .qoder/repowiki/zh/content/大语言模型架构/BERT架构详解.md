...
## 更新摘要
**变更内容**
- 新增双向文本编码器的详细技术分析，包括与GPT、ELMo的对比
- 补充Whole Word Masking技术说明及其在BERT中的应用
- 增强预训练任务的技术细节描述
- 完善BERT与其他Transformer变体的架构差异分析
...
BERT作为双向文本编码器的代表，通过自注意力机制实现真正的双向语义建模，与GPT的单向语言模型和ELMo的伪双向方法形成鲜明对比。本文还将详细介绍Whole Word Masking等关键技术的实现原理和优化效果。
...
A --> E["双向文本编码器<br/>Self-Attention机制"]
B["BERT变种<br/>bert变种.md"] --> F["RoBERTa改进<br/>更大batch/更长序列/动态mask"]
B --> G["ALBERT改进<br/>嵌入分解/跨层共享/SOP"]
B --> H["SpanBERT改进<br/>SpanMask/SBO/去NSP"]
B --> I["XLNet改进<br/>PLM/Two-Stream/Partial Prediction"]
J["Transformer架构细节<br/>Transformer架构细节.md"] --> K["编码器/解码器模块"]
J --> L["注意力机制与并行化"]
M["Transformer基础<br/>3.Transformer基础.md"] --> N["位置编码与BPE"]
A -.-> J
B -.-> J
A -.-> M
```
...
**图表来源**
- [bert细节.md:1-310](file://02.大语言模型架构/bert细节/bert细节.md#L1-L310)
...
- **双向文本编码器**：通过Self-Attention机制实现真正的双向语义建模
...
- [bert细节.md:277-307](file://02.大语言模型架构/bert细节/bert细节.md#L277-L307)
...
- **Whole Word Masking**：将被分词的WordPiece整体进行遮罩，避免部分词被遮罩导致的语义理解困难
...
- [Transformer架构细节.md:9-14](file://02.大语言模型架构/Transformer架构细节/Transformer架构细节.md#L9-L14)
- [Transformer架构细节.md:60-83](file://02.大语言模型架构/Transformer架构细节/Transformer架构细节.md#L60-L83)
- [bert细节.md:271](file://02.大语言模型架构/bert细节/bert细节.md#L271)
...
### 5) 双向文本编码器技术详解
- **双向语义建模**：通过Self-Attention机制，每个词都能同时看到上下文信息
- **技术实现**：BERT只使用Transformer的Encoder部分，通过Padding Mask实现双向信息可见性
- **对比分析**：
  - **单向模型**（GPT）：只能看到上文信息，使用Causal Mask
  - **伪双向模型**（ELMo）：左右双向LSTM分别编码后拼接
  - **真双向模型**（BERT）：一体化融合，每个词同时看到全部上下文

```mermaid
graph TB
subgraph "双向文本编码器类型"
Single["单向编码器<br/>GPT"]
Pseudo["伪双向编码器<br/>ELMo"]
True["真双向编码器<br/>BERT"]
end
subgraph "信息可见性"
Left["只看上文"]
...
### 6) Whole Word Masking技术
- **技术背景**：解决WordPiece分词导致的局部遮罩问题
- **实现原理**：将被拆分的WordPiece整体进行遮罩，确保语义完整性
- **技术优势**：避免模型只需记住词序列片段即可完成任务，提高语义理解质量

**章节来源**
- [bert细节.md:271](file://02.大语言模型架构/bert细节/bert细节.md#L271)

### 7) BERT变体：RoBERTa、ALBERT、SpanBERT、XLNet
- **RoBERTa**
...
- **双向语义建模**：通过Self-Attention实现真正的双向信息融合
...
- **双向语义理解**：确保Self-Attention正确实现，避免信息泄露

**章节来源**
- [bert细节.md:186-219](file://02.大语言模型架构/bert细节/bert细节.md#L186-L219)

## 结论
BERT以双向Transformer编码器为核心，通过MLM与NSP实现强大的上下文建模能力；变体在训练强度、遮罩策略、任务设计与参数共享等方面持续优化。理解BERT的输入嵌入、注意力机制与预训练策略，是掌握现代PLM的关键。双向文本编码器技术的发展为NLP任务提供了更强大的语义理解能力，Whole Word Masking等技术创新进一步提升了模型的语义完整性。
...
- **术语速览**
  ...
  - Whole Word Masking：整体词遮罩技术
  - 双向文本编码器：同时利用上下文信息的编码器