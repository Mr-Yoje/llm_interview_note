...
**A**: **双向文本编码器**是一种能够**同时利用上下文信息**（前文 + 后文）对文本进行编码的神经网络模型。
...
|------|------------|---------|
| **单向** | 只能看到上文 | GPT |
| **伪双向** | 左→右 + 右→左，分别编码后拼接 | ELMo |
| **真双向** | 同时看到全部上下文 | **BERT** |

**技术实现**：BERT 只使用 Transformer 的 **Encoder** 部分，通过 **Self-Attention** 机制，每个词都能"看到"句子中所有其他词。

**预训练任务**：
- **MLM**（Masked Language Model）：随机 mask 15% 的 token，通过上下文预测
- **NSP**（Next Sentence Prediction）：判断两句是否为连续句
...
| **注意力掩码** | Padding Mask（看全部） | Causal Mask（只看左边） |
| **优势任务** | 理解任务（分类、NER、QA） | 生成任务（写作、对话） |

> **一句话总结**：双向文本编码器的核心思想是**每个词的表示都融合了句子中所有其他词的信息（无论前后）**，从而获得更丰富的上下文语义。

*归档时间: 2025-02-26*

---

### BERT MLM 中 10% 随机替换 token 的含义

**Q**: 10% 的 tokens 会替换为随机 token，"在最后一层随机替换的这个 token 位去预测它真实的词"是什么意思？

**A**:

BERT MLM 对 15% 被选中 token 的处理：80% 换成 [MASK]、10% 换成随机 token、10% 保持不变。

**完整流程**：

1. 原始句子：我 喜欢 猫
2. 选中"猫"，以 10% 概率随机替换为"苹果"：我 喜欢 苹果
3. 送入 BERT 编码，模型不知道"苹果"是被替换过的
4. 在 BERT 最后一层，被替换位置（第3个位置）的输出向量经过线性层 + softmax 预测原始的真实词，目标：预测出"猫"

**"最后一层"的含义**：BERT 有 L 层 Transformer Block，每层输出一个向量。MLM 任务只取**最后一层**被替换位置的向量做预测，因为最后一层融合了最深的上下文信息。

**为什么需要 10% 随机替换**：
- 如果只有 [MASK] 替换，模型只需关注 [MASK] 位置就能做预测，不需要认真处理其他位置
- 引入随机替换后，模型**不知道哪个位置被篡改了**，被迫在**每个位置**都学好上下文表征
- 这让 BERT 获得更好的语境相关词向量，是解决**一词多义**的关键特性

**三者对比**：
- 80% [MASK]：主要训练信号，让模型学会上下文预测
- 10% 随机替换：迫使模型全局关注，增强每个位置的表征质量
- 10% 保持不变：给模型轻微 bias，稳定训练，让表征接近真实词
