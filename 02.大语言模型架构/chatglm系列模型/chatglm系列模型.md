...
**A**: **Span** 在 NLP 中的核心含义是：**文本中的一个连续片段/区间**。

**具体例子**：

```
句子：我 喜欢 深度 学习 自然 语言 处理

单个 token：[深度]           ← 长度为1的span
短 span：[深度 学习]         ← 长度为2的span
长 span：[自然 语言 处理]     ← 长度为3的span
```
...
| 模型 | 应用方式 |
|------|---------|
| **SpanBERT** | 将随机 mask 单个 token 改为 mask 连续的 span |
| **T5 / UL2** | Span Corruption：替换连续片段为特殊标记 |
| **GLM** | 对采样的 spans 进行随机打乱和自回归预测 |

**一句话总结**：Span 就是文本中的一个"连续片段"，预训练模型通过 mask 和预测这些连续片段，让模型学习到**片段级别的语义理解能力**。
...
**A**: **Corrupted 文本**（被破坏的文本）是**自编码类语言模型**预训练中的核心概念。

> **Corrupted 文本 = 原始文本经过"破坏"后，带有 [MASK] 或特殊标记的版本**
...
| 模型 | 破坏方式 | Corrupted 示例 |
|------|---------|----------------|
| **BERT** | 随机 Mask 15% token | 今天 [MASK] 真好，我们 [MASK] [MASK] 吧 |
| **T5** | Span Corruption（连续片段替换） | 今天 [X] 真好，我们 [Y] 吧 |
| **GLM** | 连续 spans 删除 + 保留位置 | 今天 [M] 真好，我们 [M] |

**核心思想**：模型是一个"去噪器"，学习从损坏的输入中恢复干净的信息。

---

### Part A 与 Part B 的区别？

**Q**: Part A 是被 mask 的文本 $x_{\text{corrupt}}$，Part B 由 masked spans 组成，哪个是 Part A？

**A**: 

| | **Part A** | **Part B** |
|---|---|---|
| **内容** | 前 5 个 token：`x1, x2, [M], x4, [M]` | 后 7 个 token：`[S], x3, [E], [S], x5, x6, [E]` |
| **作用** | 可见上下文（Corrupted Text） | 待生成的目标 spans |
| **注意力** | 内部双向可见 | 自回归，只能看历史 |

**注意力掩码规则**：

```
Part A（前5个）：  彼此可见，不可见 Part B
Part B（后7个）：  可见 Part A，自回归可见自身历史
```

> **一句话总结**：Part A 是"被破坏后的文本骨架"（提供上下文），Part B 是"需要填补的内容"（训练生成能力）。
...
**A**: **二维位置编码**是 **GLM 模型**为了解决 span 预测任务中的位置表示问题而设计的。
...
```
Part A:  x1(0), x2(1), [M](2), x4(3), [M](4)
Part B:  [S](5), x3(6), [E](7), [S](8), x5(9), x6(10), [E](11)

问题：x3 在原句中实际位于位置 2，但被标记为位置 6
      → 模型会丢失"原始位置信息"！
```
...
| 位置 id | 含义 | 作用 |
|---------|------|------|
| **Position 1** | 在 Part A 中的原始位置 | 告诉模型"这个词在原句中的哪里" |
| **Position 2** | 在 span 内部的相对位置 | 告诉模型"这是 span 的第几个字" |

**编码示例**：

| Token | Position 1 | Position 2 | 说明 |
|-------|-----------|-----------|------|
| x1 | 0 | 0 | Part A 普通 token |
| [M] | 2 | 0 | mask 占位符，对应 span 1 |
| [S]（span1） | 2 | 0 | span 1 开始，原始位置 2 |
| x3 | 2 | 1 | span 1 的第 1 个 token |
| [S]（span2） | 4 | 0 | span 2 开始，原始位置 4 |
| x5 | 4 | 1 | span 2 的第 1 个 token |
| x6 | 4 | 2 | span 2 的第 2 个 token |

> **一句话总结**：二维位置编码 = 原始位置（Position 1）+ Span 内部位置（Position 2），让 GLM 既能理解全局上下文，又能准确进行 span 内部的自回归生成。
