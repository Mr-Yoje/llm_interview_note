...
1. **「Self-Attention 本质上是不是一种特殊的全连接？」**
   → 可以理解为「动态生成权重矩阵」的全连接，但权重是输入相关的
2. **「为什么 Transformer 里既有 Attention 又有 FFN？」**
   → 职责分工：Attention 做信息聚合，FFN 做特征变换与非线性引入
3. **「Attention 的计算复杂度如何优化？」**
   → 稀疏 Attention、Flash Attention、线性 Attention 等
4. **「如果不用 Q/K，直接让 V 自己做点积会怎样？」**
   → 会退化为 point-wise 操作，失去序列建模能力
...
**Transformer 各模块的计算复杂度**

| 模块                           | 时间复杂度               | 空间复杂度       | 备注                       |
| ------------------------------ | ------------------------ | ---------------- | -------------------------- |
| **Self-Attention**       | **O(n²d)**        | **O(n²)** | 序列长度的平方是瓶颈       |
| **Multi-Head Attention** | O(n²d)                  | O(n²)           | 多头并行，复杂度与单头相同 |
| **FFN (前馈网络)**       | **O(nd²)**        | O(nd)            | d_ff 通常为 4×d_model     |
| **LayerNorm**            | O(nd)                    | O(d)             | 可忽略                     |
| **整个 Encoder Block**   | **O(n²d + nd²)** | O(n² + nd)      | Attention 通常是瓶颈       |
| **整个 Transformer**     | O(L(n²d + nd²))        | O(L(n² + nd))   | L 为层数                   |

**关键结论**：

- **Attention 是平方复杂度**，序列长度 n 是主要瓶颈
- **FFN 是线性复杂度**，但隐藏维度 d_ff 较大（通常 4×d）
- 当 n > d 时，Attention 成为主导；当 n < d 时，FFN 可能成为主导
...
| 步骤                 | 计算                                        | 复杂度            |
| -------------------- | ------------------------------------------- | ----------------- |
| Q, K, V 投影         | $XW_Q, XW_K, XW_V$                        | O(3nd²)          |
| 计算 Attention Score | $QK^T$                                    | **O(n²d)** |
| Softmax              | $\text{softmax}(\frac{QK^T}{\sqrt{d_k}})$ | O(n²)            |
| 加权求和             | $\text{Attention} \cdot V$                | O(n²d)           |
| 输出投影             | $O = \text{Concat} \cdot W_O$             | O(nd²)           |

**总时间复杂度**：O(n²d + nd²) ≈ **O(n²d)**（当 n > d 时）

**空间复杂度**：需要存储 n×n 的 Attention 矩阵 → **O(n²)**

Multi-Head Attention 的复杂度

**关键洞察**：多头注意力的复杂度与单头**相同**
...
每个头：O(n² × d/h)
h 个头总和：h × O(n² × d/h) = O(n²d)
```

FFN (Feed-Forward Network) 的复杂度
...
- 输入/输出维度：d_model = 512
- 隐藏层维度：d_ff = **2048 = 4 × d_model**

**复杂度**：

| 操作     | 计算                       | 复杂度                  |
| -------- | -------------------------- | ----------------------- |
| 第一层   | $xW_1$ (d_model → d_ff) | O(nd × d_ff) = O(nd²) |
| 激活函数 | 逐元素                     | O(nd_ff) = O(nd)        |
| 第二层   | $aW_2$ (d_ff → d_model) | O(nd_ff × d) = O(nd²) |
...
| 特性 | 说明 |
|------|------|
| **标准化输入** | 与 BatchNorm、LayerNorm 的目标一致 |
| **梯度健康** | 避免梯度消失或爆炸 |
| **数值稳定** | 激活函数（如 softmax、GeLU）在输入适中时表现最好 |
| **训练可预测** | 学习率等超参数更容易调优 |

---

### 对比：Attention 中的缩放（方向相反）

| 场景 | 操作 | 目的 |
|------|------|------|
| **Embedding** | **乘以** $\sqrt{d}$ | 放大（方差从 1/d → 1）|
| **Attention** | **除以** $\sqrt{d_k}$ | 缩小（方差从 $d_k$ → 1）|

**为什么方向不同？**
- Embedding：初始化后太小，需要**放大**
- Attention：点积后太大，需要**缩小**
```
