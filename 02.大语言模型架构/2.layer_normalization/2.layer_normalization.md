...
## RMSNorm vs LayerNorm 对比优缺点

**Q**: RMSNorm和LayerNorm对比 优缺点
...
RMSNorm 的关键改动就是**去掉了减均值 $(x - \\mu)$ 这一步**，直接用 RMS 缩放：

- **经验上**：大部分 NLP 场景中，输入的均值本身接近 0，减均值带来的收益很小
- **计算上**：省去均值计算 $\\frac{1}{n}\\sum x_i$ 和方差中 $(x_i - \\mu)^2$ 的展开，简化为直接算 $\\frac{1}{n}\\sum x_i^2$
...
**Q**: Transformer 中的 LayerNorm 怎么讲？

**A**:

**30秒极简版**

> LayerNorm 对单个 token 的特征维度做归一化（均值0方差1），不依赖 batch 大小，天然支持变长序列和推理阶段。现代 LLM 普遍采用 Pre-Norm（先归一化再进子层），LLaMA 等进一步用 RMSNorm 去掉均值计算以加速。

**2分钟标准版**

1. **做什么（15秒）**

LayerNorm 对每个 token 的特征向量做归一化，公式：y = (x - mu) / sqrt(sigma^2 + eps) * gamma + beta。其中 mu 和 sigma^2 是该 token 在特征维度上的均值和方差，gamma 和 beta 是可学习参数。

2. **为什么不用 BatchNorm（30秒）**

三个原因：
- **变长序列**：NLP 句子长度不固定，Padding 会污染 BN 的均值方差
- **推理不一致**：BN 训练时用 batch 统计量，推理时用滑动平均，训练推理不一致
- **小 batch 不稳定**：LLM 微调时 batch 往往很小，BN 统计量不可靠

3. **Pre-Norm vs Post-Norm（30秒）**

- Post-Norm（原版 Transformer）：LN 在残差之后，正则化更强，效果上限高但训练不稳定
- Pre-Norm（现代 LLM）：LN 在子层之前，恒等路径更突出，训练更稳定
- 现代 LLM（GPT-3/4, LLaMA）几乎全部用 Pre-Norm

4. **RMSNorm 的改进（20秒）**

LLaMA/Qwen/DeepSeek 用 RMSNorm 替代 LayerNorm：
- 去掉减均值操作，只用 RMS 缩放
- 省去 beta 偏置参数，参数量减少约 50%
- 计算只需一次遍历，推理加速 ~10-15%
- 效果与 LayerNorm 几乎无差异
