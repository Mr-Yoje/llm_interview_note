...
## Self-Attention 与 FFN 的角色分工及去掉的影响

**Q**: self-attention 和 FFN 分别承担什么角色，去掉会有什么影响

**A**:

### Self-Attention：通信与路由层

- **角色**：Token 间的信息交互（"谁和谁相关？"）
- **操作粒度**：跨 token（序列维度）
- **参数占比**：~1/3（W_Q, W_K, W_V, W_O）
- **关键特征**：线性操作（加权求和），本身不产生新的非线性变换能力
- **具体作用**：指代消解、上下文聚合、模式匹配、信息路由

### FFN：知识与记忆层

- **角色**：对每个 token 独立进行非线性变换，存储训练中学到的知识
- **操作粒度**：单 token（位置独立，所有 token 共享参数）
- **参数占比**：~2/3（W1: d×4d, W2: 4d×d）
- **FFN 作为 Key-Value Memory**（Geva et al., EMNLP 2021）：W1 的每一行是 Key（存储输入模式），W2 的每一行是 Value（存储对应知识）；低层存储词汇级模式，高层存储语义级知识

### 去掉 Self-Attention 的影响

- 失去 token 间通信能力，模型退化为**词袋模型**（每个 token 独立经过 FFN）
- 性能崩溃：翻译、问答等需要上下文的任务几乎不可用
- 1 层无 Attention 的 Transformer ≈ 对每个 token 跑同一个 MLP（相当于 fastText）

### 去掉 FFN 的影响

- 失去非线性变换能力和知识存储，Transformer 变成**纯线性模型**（多层可塌缩为单层线性）
- 性能大幅下降，但不像去掉 Attention 那样灾难性（Self-Attention 仍能做信息路由）
- 原始 Transformer 论文消融实验证实：去掉 FFN 比去掉 Attention 影响稍小，但仍然严重

### 协同关系

Self-Attention："把相关上下文信息聚合到当前 token" → FFN："根据上下文检索知识，做非线性变换"

|             | Self-Attention           | FFN                        |
| ----------- | ------------------------ | -------------------------- |
| 角色        | 通信/路由                | 知识/记忆                  |
| 操作维度    | 跨 token                 | 单 token                   |
| 参数占比    | ~1/3                     | ~2/3                       |
| 线性/非线性 | 线性                     | 非线性                     |
| 去掉后      | 退化为词袋模型（灾难性） | 退化为线性模型（严重退化） |

*归档时间: 2026-02-26*

---

### Transformer Input Embedding 层的作用

**Q**: Transformer input embedding层的作用是什么？

**A**:

**一句话概括**：将离散的 token ID 映射为连续的高维向量，是模型从"符号世界"进入"数学世界"的桥梁。

**三个核心作用**：

1. **离散 → 连续**：输入是 token ID（整数），如 "我" → 1057。Embedding 层通过查找表（lookup table），将每个 ID 映射为 d_model 维的浮点向量：Embedding(x) = W_E[x]，其中 W_E ∈ R^(V×d_model)，V 是词表大小，d_model 是隐藏维度（如 4096）。

2. **语义空间构建**：Embedding 矩阵 W_E 是可学习的。训练过程中，语义相近的 token 会在向量空间中靠近，模型自动学会："猫"和"狗"向量相似（同属动物），"开心"和"高兴"向量相似（同义），"猫"和"汽车"向量距离远。

3. **对齐维度**：Transformer 后续所有计算（Attention、FFN）都要求输入是 d_model 维的向量。Embedding 层将离散 ID 统一转换为 d_model 维，使后续计算成为可能。

**与位置编码的关系**：Embedding 输出 + Positional Encoding = Transformer 的最终输入。Embedding 提供"是什么词"，位置编码提供"在哪个位置"。
...
**Q**: 不使用(Q, K, V)，单纯线性层会怎么样？

**A**:

Attention 计算公式：Attention(X) = softmax(QK^T / sqrt(d_k)) V，其中 Q=XW_Q, K=XW_K, V=XW_V。

三个投影矩阵的核心意义：Q（查询）="我在找什么？"，K（键）="我有什么可以匹配的？"，V（值）="匹配到后，我提供什么信息？"

**方案一：用单一线性层替代** Y = XW

这就是一个普通的线性变换（FFN的一半），**完全丧失了 token 间的交互能力**：

| 对比 | QKV Attention | 单一线性层 |
|:---|:---|:---|
| token交互 | 有（通过QK点积计算相似度） | 无（每个token独立变换） |
| 输出依赖 | 依赖整个序列 | 只依赖当前token |
| 表达能力 | 可变（注意力权重随输入变化） | 固定（线性变换参数固定） |

**方案二：用两元组(Q, V)替代**

没有 K，Q 直接和 V 做点积计算注意力。问题：Q 和 V 来自不同空间，"查询"和"值"语义不对齐，QV 点积缺乏明确的匹配含义，训练不稳定。

**方案三：用两元组(K, V)替代**

没有 Q，K 和 V 直接做运算。问题：缺少"查询"方，无法表达"谁在关注谁"，模型无法针对不同位置产生不同的注意力模式。

**为什么 QKV 三元组是最优的**：
1. **角色分离**：Q/K/V 各自学习不同的表示空间，Q 负责主动查询，K 负责被动匹配，V 负责提供内容
2. **灵活性**：不同位置可以有不同的查询需求（Q不同），但共享匹配标准（K/V）
3. **跨注意力**：Encoder-Decoder Attention 中 Q 来自 Decoder，K/V 来自 Encoder，三元组天然支持这种不对称结构
