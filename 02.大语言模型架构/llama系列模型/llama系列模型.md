...
**面试问答补充：LLaMA-7B 的 hidden_dim 为什么是 4096？**

**Q**: 为什么 LLaMA-7B 选择 hidden_dim = 4096？这个维度是如何确定的？

**A**: 

**1. 模型缩放定律（Scaling Laws）**

LLaMA 的维度设计遵循 **Chinchilla 缩放定律**，在参数量和计算量之间取得平衡。
...
以 4096 为例：
- Attention 参数：4 × 16M × 32 ≈ 2B
- FFN 参数：2 × 4096 × 11008 × 32 ≈ 2.9B
- Embedding：32000 × 4096 ≈ 0.1B
- 总计 ≈ 6.7B (即 7B 模型)
```

**原因 3：与 attention 头数匹配**
```
hidden_dim = num_heads × head_dim
4096 = 32 × 128

这样每个头的维度是 128，是 64 的倍数（符合 Tensor Core 优化要求）
```
...
**规律**：每增加约 2 倍参数，hidden_dim 约增加 √2 倍（≈1.4 倍）
...
# 检查：
# 1. head_dim 是 64 的倍数（Tensor Core 要求）✓
# 2. hidden_dim 是 2 的幂次方 ✓
# 3. 参数量控制在 7B 左右 ✓
```

**5. 为什么是 4096 而非 4095 或 4097？**

| 维度 | 问题 |
|------|------|
| 4095 | 不是 2 的幂，硬件效率低 |
| 4097 | 质数，无法被 head 数整除 |
| **4096** | ✅ 2^12，可被 32 整除，硬件友好 |

> **一句话总结**：**4096 = 2^12**，是硬件友好的 2 的幂次方，能被 32 个 attention 头整除（每头 128 维），且能让 32 层模型的总参数量正好控制在 **~7B** 规模。这是工程实践和理论缩放定律的平衡点。
