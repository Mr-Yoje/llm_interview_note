...
## Activation Checkpointing 与 ZeRO 模型状态量详解

**Q1**: Activation Checkpointing 是什么？

**A1**:

Activation Checkpointing（激活检查点/梯度检查点）是一种"用重计算换显存"的优化技术。前向传播时不保存所有中间激活，只保存少数检查点；反向传播时从检查点重算临时激活，用完即释放。

- 节省显存：激活显存减少 70~80%
- 代价：训练速度慢 20~30%（需额外一次前向传播用于重计算）
- 策略：通常以 Transformer 大层为粒度设检查点，只存每层输入
- 与 ZeRO 配合：ZeRO 分片模型状态 → 减少固定显存；Checkpointing 减少激活 → 减少可变显存，两者互补

**Q2**: ZeRO 中"模型状态量"指的是什么？

**A2**:

模型状态量 = 参数（Parameters）+ 梯度（Gradients）+ 优化器状态（Optimizer States），即训练过程中除了中间激活以外的所有显存占用。

以 7B 模型（混合精度 AdamW）为例：
- 参数 (FP16): 14 GB
- 梯度 (FP16): 14 GB
- 优化器状态 (FP32): 84 GB（m: 28GB + v: 28GB + 权重主副本: 28GB）
- 合计: 112 GB

其中优化器状态占大头（~75%），所以 ZeRO-1 仅分片优化器状态就能获得约 4 倍显存节省。

注意：中间激活（Activations）不属于模型状态量，由 Activation Checkpointing 另外优化。

ZeRO 三个阶段分片对象：
- Stage 1: 只分片优化器状态 → 每卡显存 2Ψ + 2Ψ + 12Ψ/N
- Stage 2: 分片优化器状态 + 梯度 → 每卡显存 2Ψ + 2Ψ/N + 12Ψ/N
- Stage 3: 分片参数 + 梯度 + 优化器状态 → 每卡显存 16Ψ/N
