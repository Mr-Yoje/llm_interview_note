...
行并行（和列并行）的梯度计算确实存在跨卡依赖：

### 行并行的梯度依赖

前向：Y = XA = X1A1 + X2A2（All-Reduce 求和）

1. **对权重 A 的梯度 — 无跨卡依赖**：∂L/∂A1 = X1^T · ∂L/∂Y，GPU0 有 X1 和 ∂L/∂Y（All-Reduce 后每卡都有），可以独立计算
2. **对输入 X 的梯度 — 有跨卡依赖**：∂L/∂X = [∂L/∂X1  ∂L/∂X2]，每张卡只能算出一个分片，必须通过 All-Gather 拼接得到完整输入梯度

### 列并行的梯度依赖

前向：Y = [XA1  XA2]（All-Gather 拼接）

1. **对权重 A 的梯度 — 无跨卡依赖**：∂L/∂A1 = X^T · ∂L/∂Y1，各卡独立计算
2. **对输入 X 的梯度 — 有跨卡依赖**：∂L/∂X = ∂L/∂Y1 · A1^T + ∂L/∂Y2 · A2^T，必须 All-Reduce 求和
