...
**Q**: 模型和数据并行 参数更新时 怎么聚合

**A**:

### 数据并行（DP/DDP/FSDP）的参数聚合

数据并行的核心问题：每张卡有完整模型副本 + 不同数据 → 各卡算出不同梯度 → 如何聚合出统一参数？

1. **DP（DataParallel）— 中心化聚合**：所有梯度 gather 到 GPU0 → 取均值 → 更新 → broadcast 参数。通信原语：Gather + Broadcast。问题：GPU0 负载瓶颈、通信开销 O(N)

2. **DDP（DistributedDataParallel）— 去中心化 All-Reduce**：各卡通过 All-Reduce（Ring All-Reduce）得到相同的平均梯度，各自独立更新参数。关键假设：初始参数相同 + 梯度相同 + 优化器状态相同 → 更新后参数必然一致。通信原语：All-Reduce（1 次）

3. **FSDP/ZeRO — 分片聚合（Reduce-Scatter + All-Gather）**：将 All-Reduce 拆解为 Reduce-Scatter + All-Gather。Reduce-Scatter：梯度聚合后每卡只保留 1/N；各卡只更新自己负责的参数分片；All-Gather：收集所有分片的更新结果

| 方案 | 梯度聚合 | 参数更新 | 通信原语 | 显存 |
|------|---------|---------|---------|------|
| DP | Gather 到主卡 | 主卡更新后 Broadcast | Gather+Broadcast | 每卡全量 |
| DDP | All-Reduce 均值 | 各卡独立更新（结果一致） | All-Reduce | 每卡全量 |
| FSDP/ZeRO-1 | Reduce-Scatter | 各卡更新1/N优化器 | Reduce-Scatter + All-Gather | 优化器 1/N |
| FSDP/ZeRO-3 | Reduce-Scatter | 各卡更新1/N参数分片 | Reduce-Scatter + All-Gather | 全部 1/N |

### 模型并行的参数聚合

模型并行中，每张卡本身只拥有模型的一部分参数，不存在"梯度不同需要聚合"的问题：

1. **张量并行**：权重梯度无跨卡依赖（各卡独立计算自己的参数梯度）；但输入梯度有跨卡依赖——行并行需要 All-Gather 拼接，列并行需要 All-Reduce 求和
2. **流水线并行**：无需聚合，每张卡的参数本就是不同层的参数。通信内容：前向传激活、反向传梯度（P2P 通信）

**总结**：数据并行的核心难题是梯度聚合（不同卡算出不同梯度必须统一）；模型并行不存在梯度聚合问题（每卡参数本就是不同部分），只需在计算中同步中间结果或传递激活。
