...
- [**Continuous batching**](https://www.anyscale.com/blog/continuous-batching-llm-inference "Continuous batching")：有iteration-level的调度机制，每次迭代batch大小都有所变化，因此vLLM在大量查询下仍可以很好的工作。
  ...
  **原理细节**：传统的 static batching 在一个 batch 中所有序列都生成完毕后才能处理下一个 batch，当某些序列提前结束时，其对应的 GPU 算力处于空闲状态（padding 浪费）。Continuous batching 采用 iteration-level 调度：一旦某个序列生成了 EOS token，立即将其从 batch 中移除并释放资源，同时将新的请求插入到当前 batch 中继续生成。这样每个解码迭代步的 batch 大小都是动态变化的，GPU 利用率大幅提升。

  ```
  Static Batching:
  seq1: [tok1] [tok2] [EOS] [idle] [idle]  ← 完成后 GPU 空闲
  seq2: [tok1] [tok2] [tok3] [tok4] [tok5]  ← 最长序列决定整个 batch
  seq3: [tok1] [EOS] [idle] [idle] [idle]  ← 完成后 GPU 空闲

  Continuous Batching:
  seq1: [tok1] [tok2] [EOS]  ↓ 移除，插入新请求
  seq2: [tok1] [tok2] [tok3] [tok4] [tok5]
  seq3: [tok1] [EOS]         ↓ 移除，插入新请求
  seq4: ────────────── [tok1] [tok2] [tok3]  ← 新插入
  seq5: ───────────────────── [tok1] [tok2]  ← 新插入
  ```

- [**PagedAttention**](https://vllm.ai/ "PagedAttention")：受操作系统中虚拟内存和分页的经典思想启发的注意力算法，这就是模型加速的秘诀。

  **原理细节**：LLM 推理时需要将历史 token 的 Key/Value 缓存（KV Cache）保存在 GPU 显存中。传统方式为每个序列预分配一段连续的显存空间，但序列长度不可预测，导致两个问题：
  - **内部碎片**：预分配过长，浪费显存
  - **外部碎片**：不同序列释放后无法复用零散空间

  现有系统因此浪费了 60%-80% 的显存。PagedAttention 借鉴操作系统虚拟内存的分页机制解决此问题：

  | 操作系统概念 | PagedAttention 对应 |
  |------------|-------------------|
  | 页面（Page） | KV 块（Block），固定大小如 16 token 的 K/V |
  | 字节（Byte） | 单个 token 的 Key/Value 向量 |
  | 进程（Process） | 一个序列（Sequence）的 KV Cache |
  | 页表（Page Table） | Block Table：逻辑块→物理块的映射 |
  | 按需分配 | 生成新 token 时才分配新的物理块 |

  核心思想：**KV Cache 不需要存储在连续的物理内存中**。每个序列的逻辑 KV 块通过块表（Block Table）映射到非连续的物理块。物理块按需分配，最后一个块未填满的位置留给未来使用，内存浪费仅 < 4%。

  附加优势——**内存共享**：在并行采样（beam search / parallel sampling）中，多个输出序列共享同一 prompt，其 prompt 部分的 KV Cache 可以通过 copy-on-write 机制共享物理块，避免重复存储。

  ```
  传统方式（连续预分配）：
  seq1: [████████████░░░░░░░░] 预分配20，实际用12，浪费8
  seq2: [████████░░░░░░░░░░░░] 预分配20，实际用8，浪费12
  空闲: [       ] [       ]  ← 碎片化，无法复用

  PagedAttention（按需分块）：
  seq1: [Block0][Block1][Block2░] 逻辑连续
         ↓      ↓      ↓
  物理: [P5]   [P2]   [P7]       ← 物理不连续，按需分配
  seq2: [Block0][Block1░]         ← 共享 prompt 的 Block0
         ↓      ↓
  物理: [P5]   [P9]              ← P5 与 seq1 共享（copy-on-write）
  ```
