...
**Q**: SwiGLU 激活函数是什么？为什么现代 LLM 都用它？

**A**: 

SwiGLU = Swish + GLU，即用Swish替代GLU中的Sigmoid门控函数。LLaMA、Qwen、DeepSeek、Mistral等标配。

公式：$\text{SwiGLU}(x, W, V, b, c) = (xW + b) \otimes \text{Swish}(xV + c)$

在 FFN 中的完整结构：
- 传统 FFN（ReLU）：$x \to W_1 \to \text{ReLU} \to W_2 \to$ 输出（2 个矩阵，隐藏维度 $4d$，参数 $8d^2$）
- SwiGLU FFN：$x \to W_{\text{gate}}(\text{Swish}) \otimes W_{\text{up}} \to W_{\text{down}} \to$ 输出（3 个矩阵，隐藏维度 $\frac{8d}{3}$，参数 $8d^2$）

隐藏维度从 $4d$ 调整为 $\frac{8d}{3}$ 的原因：3 个矩阵参数量 $3dh$，令 $3dh = 8d^2$ 得 $h = \frac{8d}{3}$。

SwiGLU 优于 ReLU/GELU 的原因：
1. 门控机制增强表达能力：gate 分支学习"选哪些特征通过"，类似 LSTM 门控
2. Swish 非单调性：负区有下界（约 $-0.28$），更平滑，梯度更稳定，不会"神经元死亡"
3. 实验验证：GLU 论文中 SwiGLU（BLEU 27.9）优于 GeGLU（27.8）、Swish（27.2）、GELU（27.1）、ReLU（26.8）

PyTorch 实现核心：$W_{\text{down}}(\text{F.silu}(W_{\text{gate}}(x)) \otimes W_{\text{up}}(x))$，其中 $\text{F.silu} = \text{Swish}(\beta=1)$

三个矩阵不能合并的原因：gate 分支需过 Swish 后与 up 分支逐元素乘，中间有非线性操作 $\text{Swish}(W_{\text{gate}}x) \otimes W_{\text{up}}x$，无法简化为单次矩阵乘。

LLaMA还移除了所有偏置(bias=False)：大规模训练中偏置影响极小，减少参数量，与RMSNorm配合时偏置冗余。
