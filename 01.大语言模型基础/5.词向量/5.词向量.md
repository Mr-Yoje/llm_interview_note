...
## 词表新加词 Embedding 层需要重新训练吗

**Q**: 词表新加词 embedding 层 需要重新训练吗

**A**:

词表新增词后，Embedding 矩阵从 [V, d] 变为 [V+N, d]，新增的 N 行向量没有经过训练，是随机初始化的，**必须重新训练新加的行，但不需要重新预训练整个模型**。

### 初始化策略

| 策略 | 方法 | 适用场景 |
|------|------|---------|
| 子词平均 | 用新词的 subword embedding 取平均 | 新词可被 BPE/SentencePiece 分解（最推荐） |
| 同义词复制 | 用语义最接近的已有词 embedding | 有明确近义词时 |
| Mean Embedding | 用所有已有词 embedding 的均值 | 最稳妥的默认策略 |
| 随机初始化 | 正态分布/均匀分布随机赋值 | 新词与已有词无关联时 |

### 训练策略

- **少量新词（<100）**：只训练新加的 Embedding 行，冻结其余参数
- **中等规模（100-1000）**：Embedding + 顶层几层微调
- **大规模/新领域**：LoRA 等全模型微调

### 关键 API

`model.resize_token_embeddings(len(tokenizer))` 会自动保留已有 embedding 行，新增行用默认方式初始化。同时 lm_head 也会相应扩展。

### 常见误区

- 新加词必须重新预训练整个模型 → 不需要，只需微调 embedding + 部分层
- lm_head 不需要处理 → 错误，lm_head 同样需要扩展
- 词表越大越好 → 词表过大增加参数量和 softmax 计算开销
