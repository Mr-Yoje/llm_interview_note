...
## 为什么从较低的 Transformer 层删除 Adapter 不影响太多性能

**Q**: 为什么通过从较低的 Transformer 层删除可变数量的adapter 不会影响太多性能

**A**:

### 核心原因：Transformer 不同层的功能分工不同

1. **低层编码通用底层特征**：词性、句法等跨任务共享的特征，不需要任务特定的适配
2. **高层编码任务特定语义**：与下游任务强相关，需要 Adapter 进行适配

### 三层解释

- **特征分工**：低层学通用词法/句法，高层学任务语义。低层特征跨任务共享，Adapter 对低层修改本来就小
- **Adapter 学得少**：低层 Adapter 接收的梯度信号弱（反向传播经多层衰减），学到的适配量有限
- **残差保护**：Adapter 公式 h = h + f(hW_down)W_up，删除低层 Adapter ≈ 回到恒等映射 ≈ 恢复预训练模型低层行为，而预训练低层本身就足够好

### 实验数据（AdapterDrop 论文）

- 删除前 5 层 Adapter：性能下降 < 1%，推理加速 39%
- 删除前 10 层 Adapter：性能下降约 2-3%
- 删除高层 Adapter：性能显著下降
...
| 现象 | 共同本质 |
|------|---------|
| AdapterDrop 低层可删除 | 低层编码通用特征，与任务关联弱 |
| AdaLoRA 高层分配高秩 | 高层权重矩阵重要性更高 |
| LoRA 只训练 Attention | FFN 更"通用"，Attention 更"任务相关" |
| ALBERT 跨层参数共享 | 低层特征通用，甚至可以共享参数 |
