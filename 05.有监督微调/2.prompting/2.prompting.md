...
**Q**: P-Tuning 和 P-Tuning v2 分别是什么？有什么区别？

**A**: 

**P-Tuning v1**（论文：GPT Understands, Too, 2021）：
- 解决问题：人工设计prompt对结果影响大，离散token搜索空间大成本高
- 核心思路：将离散prompt token替换为可学习的连续Embedding
- 关键设计：用LSTM+MLP编码virtual token（Prompt Encoder），建模token间关联
- 位置灵活：virtual token可插入任意位置，不必只在前面
- 仅输入层：只在Embedding层加入，不在每层都加
- 可训练参数：约0.01%
- 问题：小模型效果差，序列标注任务效果差

**P-Tuning v2**（论文：P-Tuning v2: Prompt Tuning Can Be Comparable to Fine-tuning, 2022）：
- 核心改进：在每一层都加入Prompt tokens（深度提示优化，类似Prefix Tuning）
- 四项改进vs v1：
  1. Prompt深度：仅输入层 -> 每层都加（参数0.01% -> 0.1-3%）
  2. 重参数化：移除LSTM+MLP（发现改进很小）
  3. Prompt长度：按任务调整（简单约20，复杂约100）
  4. 分类方式：Verbalizer -> 传统分类头（更通用，适配序列标注）

Prompt Tuning家族对比：
| 维度 | Prompt Tuning | P-Tuning v1 | P-Tuning v2 | Prefix Tuning |
|------|-------------|------------|------------|-------------|
| 插入层 | 仅输入层 | 仅输入层 | 每层 | 每层 |
| 插入位置 | 仅前缀 | 灵活 | 前缀 | 前缀 |
| 重参数化 | 无 | LSTM+MLP | 无 | MLP |
| 可训练参数 | 约0.01% | 约0.01% | 约0.1-3% | 约0.1% |
| 小模型效果 | 差 | 差 | 约等于全量微调 | 好 |
